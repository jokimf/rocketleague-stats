...
class GraphQueries:
    ...
    def performance_graph(
        conn, total_games_count: int, active_players: list[str]
    ) -> dict:
        data_list = []
        with conn.cursor() as cursor:
    ...
    @staticmethod
    def score_distribution_graph(conn, active_players: list[str]) -> dict:
        datasets = []
        with conn.cursor() as cursor:
            for player_id in active_players:
                cursor.execute(
        ...
        graph = GraphBuilder().withType("line").withLabels(labels)
