...
def conditional_formatting(color: str, value: float, minimum: int, maximum: int) -> str:
    ...
    if color is None or (type(value) is not float and type(value) is not int):
        raise ValueError(f"Color can not be None and value needs to be int or float: color={color}, value={type(value)}")
...
class User:
    ...
    def is_premium(self, conn) -> bool:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT premium FROM users WHERE username=%s AND identifier=%s AND userID=%s", (self.username, self.identifier, self.userid)
            )
    ...
    def check_credentials(self, conn) -> bool:
        with conn.cursor() as cursor:
            if not self.has_valid_values():
                return False
            cursor.execute("SELECT 1 FROM users WHERE username=%s AND identifier=%s AND userid=%s", (self.username, self.identifier, self.userid))
...
def enforce_max_size(content_length: int = Header(None)):
    if content_length is None:
        raise HTTPException(status_code=411, detail="Missing Content-Length header")
