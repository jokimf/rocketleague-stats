...
class RandomFactQueries(BackendConnection):
    ...
    def generate_random_facts(self) -> list:
        union = self.record_session() + \
            self.milestone_facts() + \
            self.last_session_facts() + \
            self.last_month_summary() + \
            self.result_facts() + \
            self.game_count_facts() + \
            self.close_to_record() + \
            self.outclassed() + \
            self.at_least_1_streak() + \
            self.streak()
    ...
    def last_month_summary(self) -> list[tuple]:
        ...
        if 5 >= games_this_month > 0 and datetime.today().day < 5:
            ...
            facts.append(
                (f"Last month, you played {game_amount} games, which ranks at {rank} out of {len(month_gc)}.", 5))
        return facts
    ...
    def result_facts(self) -> list[tuple]:
        def results_table() -> list[Any]:
            self.c.execute("SELECT goals, against, COUNT(g.gameID) AS c FROM games g GROUP BY goals, against ORDER BY 1, 2")
            return self.c.fetchall()
    ...
    def close_to_record(self) -> list[tuple]:
        ...
        record_data = [(self.r.highest_stat_value_one_game("score", limit).data,
                       ...
                       (self.r.least_points_with_goals(limit).data,
