...
class Dashboard:
    ...
    def reload(self, conn):
        ...
        self.session_information = RandomFactQueries.session_data_by_date(
            conn, self.session_details["latest_session_date"]
        )
        ...
        self.last_games = RLQueries.last_x_games_stats(
            conn, active_player_ids, len(RLQueries.games_from_session_date(conn)), False
        )
        ...
        self.session_game_details = RLQueries.last_x_games_stats(
            conn, active_player_ids, self.session_game_amount, False
        )
        ...
        self.visualizations = visualizations.get_all_visualizations(
            conn, self.total_games, active_player_ids
        )
        # Games
        self.last_100_games_stats = RLQueries.last_x_games_stats(
            conn, active_player_ids, 100, True
        )
...
class Stats:
    ...
    def reload(self):
        self.total_games = GeneralQueries.total_games(self.conn)
        self.active_player_ids = GeneralQueries.get_active_player_ids(self.conn)
