...
class RecordQueries:
    @staticmethod
    def highest_stat_value_one_game(conn, stat: str, limit: int = 3) -> Record:
    ...
    @staticmethod
    def performance_records(conn, stat: str, minmax: str, limit: int = 3) -> Record:
        if stat not in [
            "score",
            "goals",
            "assists",
            "saves",
            "shots",
        ] or minmax not in ["MIN", "MAX"]:
            raise ValueError(
                f"{stat} is not in possible stats or {minmax} is not MIN or MAX"
            )
        if stat == "goals":
    ...
    def most_against(conn, limit: int = 3) -> Record:
        with conn.cursor() as cursor:
            ...
            return Record(
                "Most goals conceded by team", "mostagainst", cursor.fetchall()
            )
    ...
    def most_against_and_won(conn, limit: int = 3) -> Record:
        with conn.cursor() as cursor:
            ...
            return Record(
                "Most goals conceded but still won", "mostagainstwon", cursor.fetchall()
            )
    ...
    def most_goals_and_lost(conn, limit: int = 3) -> Record:
        with conn.cursor() as cursor:
            ...
            return Record(
                "Most goals scored and still lost", "mostscoredlost", cursor.fetchall()
            )
    ...
    def most_total_goals(conn, limit: int = 3) -> Record:
        with conn.cursor() as cursor:
            ...
            return Record(
                f"Most total goals in one game", "mosttotalgoals", cursor.fetchall()
            )
    ...
    def most_points_without_goal(conn, limit: int = 3) -> Record:
        with conn.cursor() as cursor:
            ...
            return Record(
                "Most points with no goal", "mostpointsnogoal", cursor.fetchall()
            )
    ...
    def generate_record_games(conn) -> tuple:
        ...
        records = (
            (
                RecordQueries.highest_stat_value_one_game(conn, "score", game_amount),
                RecordQueries.highest_stat_value_one_game(conn, "goals", game_amount),
                RecordQueries.highest_stat_value_one_game(conn, "assists", game_amount),
                RecordQueries.highest_stat_value_one_game(conn, "saves", game_amount),
                RecordQueries.highest_stat_value_one_game(conn, "shots", game_amount),
            ),
            (
                RecordQueries.highest_stat_team_one_game(conn, "score", game_amount),
                RecordQueries.highest_stat_team_one_game(conn, "goals", game_amount),
                RecordQueries.highest_stat_team_one_game(conn, "assists", game_amount),
                RecordQueries.highest_stat_team_one_game(conn, "saves", game_amount),
                RecordQueries.highest_stat_team_one_game(conn, "shots", game_amount),
            ),
            (
                RecordQueries.performance_records(conn, "score", "MAX", game_amount),
                RecordQueries.performance_records(conn, "goals", "MAX", game_amount),
                RecordQueries.performance_records(conn, "assists", "MAX", game_amount),
                RecordQueries.performance_records(conn, "saves", "MAX", game_amount),
                RecordQueries.performance_records(conn, "shots", "MAX", game_amount),
            ),
            (
                RecordQueries.performance_records(conn, "score", "MIN", game_amount),
                RecordQueries.performance_records(conn, "goals", "MIN", game_amount),
                RecordQueries.performance_records(conn, "assists", "MIN", game_amount),
                RecordQueries.performance_records(conn, "saves", "MIN", game_amount),
                RecordQueries.performance_records(conn, "shots", "MIN", game_amount),
            ),
            (
                RecordQueries.most_against(conn, game_amount),
                RecordQueries.most_against_and_won(conn, game_amount),
                RecordQueries.most_goals_and_lost(conn, game_amount),
                RecordQueries.most_total_goals(conn, game_amount),
            ),
            (
                RecordQueries.highest_points_nothing_else(conn, game_amount),
                RecordQueries.most_points_without_goal_or_assist(conn, game_amount),
                RecordQueries.most_points_without_goal(conn, game_amount),
                RecordQueries.least_points_at_least_1(conn, game_amount),
                RecordQueries.least_points_with_goals(conn, game_amount),
            ),
            (
                RecordQueries.diff_mvp_lvp(conn, "DESC", game_amount),
                RecordQueries.diff_mvp_lvp(conn, "ASC", game_amount),
                RecordQueries.most_solo_goals(conn, game_amount),
            ),
        )
        return records
